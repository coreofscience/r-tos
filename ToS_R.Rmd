...
I can read several files in only one line

```{r}
df_clusters_1 <- 
  read_csv("data/Cluster_1.csv") %>% 
  select(v_name) %>% 
...
safe_webscraping <- safely(oadoi_fetch)
safe_webscraping_discard <- function(dois) {
  map(raw_data_1$doi, 
      safe_webscraping, 
      email = email) %>% 
    discard(is.null)
}
titles <- map(raw_data_1$doi,
              safe_webscraping_discard)

webscraping <- 
  map(raw_data_1$doi[1], safe_webscraping, email = email) %>% 
  dplyr::select(title)
